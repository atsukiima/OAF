...
public class ShowPAreaTaxonomySelection extends ShowAbNSelection {
    ...
    protected void loadDataSource(boolean doLoad, LoadStatusDialog loadStatusDialog) {
        ...
        try {
            ...
            PAreaTaxonomyFactory factory = null;
            Hierarchy<SCTConcept> hierarchy = null;

            if (useStatedRelationships) {
                SCTReleaseWithStated statedDataSource = (SCTReleaseWithStated) dataSource;
                
                Hierarchy<SCTConcept> ancestorHierarchy = statedDataSource.getStatedHierarchy().getAncestorHierarchy(localConcept);
                
                if(ancestorHierarchy.getChildren(ancestorHierarchy.getRoot()).size() > 1) {
                    System.err.println("Unknown subhierarchy error...");
                    
                } else {
                    SCTConcept topSubhierarchyConcept = ancestorHierarchy.getChildren(ancestorHierarchy.getRoot()).iterator().next();
                    
                    factory = new SCTStatedRelationshipsPAreaTaxonomyFactory(statedDataSource, topSubhierarchyConcept);
                ...
                factory = new SCTInferredPAreaTaxonomyFactory(dataSource, hierarchy);
            }
